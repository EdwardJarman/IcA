...
function resolveVariant(
  ...
  if (variant === "black")
    return { body: BLACK.color, eye: BLACK.eyeColor, palette: WEBGL_COLORS };
  if (variant === "webgl") {
    return {
      body: WHITE.color,
      eye: eyeColor ?? "#0C0C10",
      palette: colors?.length ? colors : WEBGL_COLORS,
    };
  }
  if (variant === "custom") {
    return {
        ...
        <Defs>
          <RadialGradient id={sphereId} cx="34%" cy="26%" rx="70%" ry="70%">
            <Stop
              offset="0"
              stopColor="#FFFFFF"
              stopOpacity={variant === "black" ? 0.24 : 0.7}
            />
            <Stop offset="0.48" stopColor={resolved.body} />
            <Stop
              offset="1"
              stopColor="#07080A"
              stopOpacity={variant === "white" ? 0.22 : 0.5}
            />
          </RadialGradient>
          <LinearGradient id={webglId} x1="0" y1="0" x2="1" y2="1">
            <Stop
              offset="0"
              stopColor={resolved.palette[0] ?? WEBGL_COLORS[0]}
            />
            <Stop
              offset="0.52"
              stopColor={resolved.palette[1] ?? WEBGL_COLORS[1]}
            />
            <Stop
              offset="1"
              stopColor={resolved.palette[2] ?? WEBGL_COLORS[2]}
            />
          </LinearGradient>
        </Defs>
        <Circle
          ...
          cy="50"
          r="43"
          fill={variant === "webgl" ? `url(#${webglId})` : `url(#${sphereId})`}
        />
        <Circle
          cx="37"
          cy="28"
          r="13"
          fill="#FFFFFF"
          opacity={variant === "black" ? 0.08 : 0.2}
        />
