...
import { providerLabel, type AiProvider } from "@/lib/ai-provider";
...
export function AiProviderSwitch() {
  ...
  const chatGPTConnected = Boolean(catalog.data?.models.some((model) => model.id.startsWith("chatgpt:")));

  const choose = (provider: AiProvider) => {
    if (provider === "chatgpt" && !chatGPTConnected) {
      Alert.alert("Connect ChatGPT first", "Use the ChatGPT card below to connect your plan, then select ChatGPT here.");
      return;
    }
  ...
  return (
    <Card style={{ gap: 12 }}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 11 }}>
        <View style={{ width: 38, height: 38, borderRadius: 13, backgroundColor: colors.surfaceAlt, alignItems: "center", justifyContent: "center" }}>
          <MaterialIcons name="route" size={19} color={colors.textSoft} />
        </View>
        <View style={{ flex: 1, minWidth: 0 }}>
          <Text style={{ color: colors.text, fontSize: 14, fontWeight: "700" }}>Default AI provider</Text>
          <Text style={{ color: colors.textFaint, fontSize: 11.5, marginTop: 2 }}>All Bot messages use {providerLabel(aiProvider)} until you switch.</Text>
        </View>
      </View>
      ...
      {aiProvider === "chatgpt" ? (
        ...
        <View style={{ flexDirection: "row", gap: 7, alignItems: "center" }}>
          <MaterialIcons name="info-outline" size={14} color={colors.textFaint} />
          <Text style={{ flex: 1, color: colors.textFaint, fontSize: 10.5, lineHeight: 15 }}>Uses Rook’s shared free OpenRouter allowance.</Text>
        </View>
      )}
    </Card>
  );
