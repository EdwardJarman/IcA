...
const PREFERENCES_KEY = "luma-workroom-notification-preferences-v2";
const INSTALLATION_KEY = "luma-workroom-installation-id-v1";
...
async function createAndroidChannel() { if (Platform.OS === "android") await Notifications.setNotificationChannelAsync("workroom-alerts", { name: "Workroom alerts", description: "Task completions and approval requests from Luma Workroom.", importance: Notifications.AndroidImportance.HIGH, vibrationPattern: [0, 160, 110, 160], lightColor: "#77F3C4" }); }
...
export function useLumaNotifications() { const context = useContext(NotificationContext); if (!context) throw new Error("useLumaNotifications must be used inside LumaNotificationProvider"); return context; }
