...
export const emptyWorkroomSnapshot = (): WorkroomCloudSnapshot => ({ selectedBotId: "", onboardingComplete: false, bots: [], messages: [], tasks: [], skills: [], routines: [], approvals: [], files: [], notifications: [], activity: [] });

export const normalizeWorkroomSnapshot = (value: unknown): WorkroomCloudSnapshot => {
  const source = value && typeof value === "object" ? value as Record<string, unknown> : {};
  const array = (key: keyof Omit<WorkroomCloudSnapshot, "selectedBotId" | "onboardingComplete">) => Array.isArray(source[key]) ? source[key] : [];
